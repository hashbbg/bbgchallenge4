...
var player = {
  ...
  , speed   : 0
  , maxSpeed: 7
  , angle   : Math.PI    // in radians - 0 straight up
  , grip    : 1
  , image   : null
  ...
  , clean   : function(){
        ...
        this.context.clearRect(old.x - this.width - this.camera.x
            , old.y - this.width - camera.y
            , this.width * 4
            , this.height * 4)
  ...
  , render  : function(){
        ...
        this.context.translate(this.x + halfwidth - camera.x, this.y + halfheight -camera.y)
        this.context.rotate(this.angle)
        this.context.fillRect(-halfwidth, -halfheight, this.width, this.height)
  ...
  , update: function(){
        ...
        if ( keyPoll.left ) this.angle -= 0.05 * this.grip
        if ( keyPoll.right) this.angle += 0.05 * this.grip
        ...
        camera.x = this.x - this.offsetX
        camera.y = this.y - this.offsetY

        if ( getColor(pathData, ~~(this.x + (this.width / 2)), ~~(this.y + (this.height / 2)), 1) !== 0 ) 
            this.kill()
...
var createLevel = function(levelConf){

    // create player
    var p = _.extend(Object.create(player)
            , lucid.emitter()
            ,{
                x: levelConf.playerX
              , y: levelConf.playerY
    ...
    var levelObj = _.extend({
        cleanup: function(){ clearInterval(loop)  }
    }, lucid.emitter())
